...
// Writes to /proc/sys/net via sysctl. Non-fatal on failure (non-root, containers, etc.)
func applySystemTCPTuning() {
	...
	if applied > 0 {
		log.Printf("[SYS] TCP tuning applied (%d/%d ok)", applied, len(tunables))
	}
}
...
func (s *Session) Close() {
	if s.ic.CompareAndSwap(false, true) {
		close(s.cc)
		s.wg.closed.Store(true) // 绾喕绻氱粩瀣祮閹锋帞绮烽弬鎵畱 Add() 鐠囬攱鐪?
		s.sm.Lock()
		...
		s.cr.Close()
		// 閺嶇绺炬穱顔碱槻 1閿涙艾浜ゆ惔鏇犘╅梽銈咁嚤閼?fdl 濮橀晲绠欓崘鑽ょ波閻?s.wg.Wait() 濮濆鏀ｆ禒锝囩垳
	}
}
...
func (srv *Server) hs(cn net.Conn) {
	...
	// 娣囶喖顦查敍姘杹鐎硅姤妞傞柦鐔锋倱濮濄儵妾洪崚璺哄煂120缁夋帪绱濋梼鍙夘剾鏉烆垵鐭鹃悽杈ㄦ闂傚瓨绱撶粔璇差嚤閼峰娈戠粔鎺撴絻閹凤附鍩?
	if timeDiff > 120000 {
		log.Printf("[SRV] 閹锋帞绮锋潻鐐村复: 鐎广垺鍩涚粩顖涙闂傚瓨鍩戠拠顖氭▕鏉╁洤銇?(diff=%dms)閿涗浇顕Λ鈧弻銉ヨ嫙閸氬本顒炵€广垺鍩涚粩顖濐啎婢?婵′窘penWrt)閻ㄥ嫭妞傞梻杈剧磼", timeDiff)
		cn.Close()
		return
	}
...
func (srv *Server) hcWithPreReg(s *Session, tc *targetConn, p []byte) {
	...
	// 闂冨弶涓嶇€广垺鍩涚粩顖炩偓蹇旀娴狅絿鎮婂鏇炲絺閻ㄥ嫬娲栭悳顖涙暰閸?(Routing Loop)
	configMu.RLock()
	...
	if isSelfPort && isLocalIP(resolvedHost) {
		log.Printf("[SRV] 棣冩磧 閹锋帞绮烽崶鐐靛箚閹枫劌褰? 閹凤附鍩呴崚鏉款吂閹撮顏崣鎴濈窔閺堝秴濮熼崳銊ㄥ殰闊偆娲冮崥顒傤伂閸欙絿娈戝璇叉儕閻滎垵顕Ч?-> %s:%d", resolvedHost, pt)
		s.tm.Remove(tc.id)
		srv.stc(s, encodeFrame(2, tc.id, nil))
		return
	}
...
const serverHTML = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>SuperYellow Proxy Panel</title><script src="https://cdn.tailwindcss.com"></script><script src="https://unpkg.com/vue@3/dist/vue.global.prod.js"></script><style>body{background-color:#f9fafb;color:#111827;font-family:system-ui,-apple-system,sans-serif} .ring{border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:inset 0 0 10px rgba(0,0,0,0.05)} .copy-btn{cursor:pointer;padding:4px 12px;border-radius:6px;font-size:12px;font-weight:700;transition:all 0.2s} .copy-btn:hover{transform:scale(1.05)} .toast{position:fixed;top:20px;right:20px;background:#10b981;color:white;padding:12px 24px;border-radius:10px;font-weight:700;z-index:9999;animation:fadeIn 0.3s} @keyframes fadeIn{from{opacity:0;transform:translateY(-10px)}to{opacity:1;transform:translateY(0)}} .modal-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center;z-index:50;backdrop-filter:blur(4px)} .config-box{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:12px;font-family:monospace;font-size:12px;white-space:pre-wrap;word-break:break-all;max-height:200px;overflow-y:auto;user-select:text}</style></head><body><div id="app" class="min-h-screen flex"><div v-if="!log" class="m-auto bg-white p-8 rounded-xl shadow-2xl w-96 border border-gray-100"><h2 class="text-2xl font-black mb-8 text-center tracking-tight">SuperYellow<span class="text-yellow-400">/</span><span class="text-gray-400">Admin</span></h2><div class="space-y-4"><input v-model="u" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black transition" placeholder="缁狅紕鎮婇崨妯垮閸?><input v-model="p" type="password" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black transition" placeholder="缁狅紕鎮婄€靛棛鐖? @keyup.enter="li"><button @click="li" class="w-full bg-black text-white p-3 rounded font-bold hover:bg-gray-800 transition shadow-lg">閻ц缍嶉棃銏℃緲</button></div></div><template v-else><div class="w-64 bg-black text-white p-6 flex flex-col shadow-2xl z-10" style="min-width:256px"><div class="text-2xl font-black mb-10 tracking-tighter">SuperYellow<span class="text-yellow-400 text-3xl leading-none">.</span></div><div class="space-y-2 flex-1"><div @click="t='s'" :class="t=='s'?'bg-gray-800 text-white':'text-gray-400 hover:text-white hover:bg-gray-900'" class="p-3 cursor-pointer rounded transition font-medium">缁崵绮洪悩鑸碘偓?/div><div @click="fu();t='u'" :class="t=='u'?'bg-gray-800 text-white':'text-gray-400 hover:text-white hover:bg-gray-900'" class="p-3 cursor-pointer rounded transition font-medium">鐎广垺鍩涚粻锛勬倞</div><div @click="fc();t='c'" :class="t=='c'?'bg-gray-800 text-white':'text-gray-400 hover:text-white hover:bg-gray-900'" class="p-3 cursor-pointer rounded transition font-medium">閼哄倻鍋ｉ柊宥囩枂</div></div><div class="text-xs text-gray-600 mt-auto">SuperYellow Proxy v6</div></div><div class="flex-1 p-10 overflow-y-auto" style="width:100%"><div v-if="t=='s'" class="space-y-8"><h2 class="text-3xl font-bold mb-6 tracking-tight">缁崵绮洪悩鑸碘偓?/h2><div class="bg-white p-8 shadow-sm rounded-2xl border border-gray-100 flex justify-around items-center text-center flex-wrap"><div class="flex flex-col items-center"><div class="w-32 h-32 ring mb-4" style="width:128px;height:128px;background:conic-gradient(#3b82f6 30%, #f3f4f6 0)"><div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center" style="width:112px;height:112px;background-color:white;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;"><span class="text-xl font-bold text-gray-800">{{st.cpu}}</span></div></div><div class="text-sm font-bold text-gray-500 tracking-wider">CPU</div></div><div class="flex flex-col items-center"><div class="w-32 h-32 ring mb-4" style="width:128px;height:128px;background:conic-gradient(#10b981 40%, #f3f4f6 0)"><div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center" style="width:112px;height:112px;background-color:white;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;"><span class="text-xl font-bold text-gray-800">{{st.mem}}</span></div></div><div class="text-sm font-bold text-gray-500 tracking-wider">閸愬懎鐡?/div></div><div class="flex flex-col items-center"><div class="w-32 h-32 ring mb-4" style="width:128px;height:128px;background:conic-gradient(#8b5cf6 60%, #f3f4f6 0)"><div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center" style="width:112px;height:112px;background-color:white;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;"><span class="text-xl font-bold text-gray-800">{{st.go}}</span></div></div><div class="text-sm font-bold text-gray-500 tracking-wider">閸楀繒鈻?/div></div></div><div class="grid grid-cols-3 gap-6" style="display:flex;flex-wrap:wrap;gap:1.5rem"><div class="bg-white p-6 shadow-sm rounded-2xl border border-gray-100" style="flex:1;min-width:200px"><div class="text-sm font-bold text-gray-400 mb-1">鏉╂劘顢戦弮鍫曟？</div><div class="text-2xl font-black text-gray-800">{{st.ut}}</div></div><div class="bg-white p-6 shadow-sm rounded-2xl border border-gray-100" style="flex:1;min-width:200px"><div class="text-sm font-bold text-gray-400 mb-1">娑撳﹨顢戝ù渚€鍣?/div><div class="text-2xl font-black text-blue-600">{{st.tx}}</div></div><div class="bg-white p-6 shadow-sm rounded-2xl border border-gray-100" style="flex:1;min-width:200px"><div class="text-sm font-bold text-gray-400 mb-1">娑撳顢戝ù渚€鍣?/div><div class="text-2xl font-black text-green-600">{{st.rx}}</div></div></div></div><div v-if="t=='u'"><div class="flex justify-between items-center mb-6"><h2 class="text-3xl font-bold tracking-tight">鐎广垺鍩涚粻锛勬倞</h2><div class="flex gap-3"><button @click="rg" class="bg-yellow-400 text-black px-5 py-2 rounded-lg font-bold shadow-lg hover:bg-yellow-300 transition">棣冨箟 闂呭繑婧€閻㈢喐鍨氱€广垺鍩?/button><button @click="sa=true;isEdit=false;nu={enable:true,LimitGb:0,ExpStr:''}" class="bg-black text-white px-6 py-2 rounded-lg font-bold shadow-lg hover:bg-gray-800 transition">+ 閹靛濮╁ǎ璇插</button></div></div><div class="bg-white shadow-sm rounded-2xl border border-gray-100 overflow-hidden"><table class="w-full text-left text-sm"><thead class="bg-gray-50 text-gray-500 border-b border-gray-100"><tr><th class="p-4 font-bold">閻樿埖鈧?/th><th class="p-4 font-bold">閻劍鍩涢崥?/th><th class="p-4 font-bold">鐎靛棛鐖?/th><th class="p-4 font-bold">閸掔増婀￠弮?/th><th class="p-4 font-bold">濞翠線鍣?/th><th class="p-4 font-bold">TX / RX</th><th class="p-4 font-bold text-right">閹垮秳缍?/th></tr></thead><tbody><tr v-for="u in us" class="border-b border-gray-50 hover:bg-gray-50 transition"><td class="p-4"><span v-if="u.enable" class="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-bold">濞茶濮?/span><span v-else class="px-2 py-1 bg-red-100 text-red-700 rounded text-xs font-bold">鐏忎胶顩?/span></td><td class="p-4 font-bold text-gray-800">{{u.username}}</td><td class="p-4 font-mono text-gray-500 text-xs">{{u.password}}</td><td class="p-4 text-gray-600">{{u.Exp}}</td><td class="p-4 text-gray-600"><span v-if="u.limit>0">{{(u.limit/1073741824).toFixed(1)}}G</span><span v-else class="text-gray-400">閺冪娀妾?/span></td><td class="p-4 font-mono text-gray-500 text-xs">{{u.Tx}} / {{u.Rx}}</td><td class="p-4 text-right"><button @click="cp(u,'vless')" class="copy-btn bg-blue-100 text-blue-700 hover:bg-blue-200 mr-2">棣冩惖 VLESS</button><button @click="cp(u,'json')" class="copy-btn bg-purple-100 text-purple-700 hover:bg-purple-200 mr-2">棣冩惖 JSON</button><button @click="eu(u)" class="text-blue-500 hover:text-blue-700 font-bold transition mr-3">缂傛牞绶?/button><button @click="du(u.id)" class="text-red-500 hover:text-red-700 font-bold transition">閸掔娀娅?/button></td></tr></tbody></table></div></div><div v-if="t=='c'" class="space-y-6 max-w-3xl"><h2 class="text-3xl font-bold tracking-tight mb-6">閼哄倻鍋ｉ柊宥囩枂</h2><div class="bg-white p-8 shadow-sm rounded-2xl border border-gray-100 space-y-5"><div class="grid grid-cols-2 gap-6" style="display:flex;gap:1.5rem"><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">闂堛垺婢樼拹锕€褰?/label><input v-model="cf.panel_user" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">闂堛垺婢樼€靛棛鐖?/label><input v-model="cf.panel_pass" type="password" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div></div><div><label class="block text-sm font-bold text-gray-500 mb-2">閸╃喎鎮?(閻ｆ瑧鈹栭懛顏嗩劮鐠囦椒鍔?</label><input v-model="cf.domain" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black" placeholder="your.domain.com"></div><div class="grid grid-cols-2 gap-6" style="display:flex;gap:1.5rem"><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">閻╂垵鎯夌粩顖氬經</label><input v-model="cf.listen_ports" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">娴碱亣顥婂Ο鈥崇础</label><select v-model="cf.camo_mode" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"><option value="proxy">閸欏秴鎮滄禒锝囨倞</option><option value="self">閼奉亜缂撳▎銏ｇ箣妞?/option><option value="local">鏉╂柨娲?00</option></select></div></div><div v-if="cf.camo_mode=='proxy'"><label class="block text-sm font-bold text-gray-500 mb-2">娴狅絿鎮婇惄顔界垼</label><input v-model="cf.camo_target" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black" placeholder="www.microsoft.com:443"></div><button @click="sc" class="bg-black text-white px-8 py-3 rounded-lg font-bold shadow-lg hover:bg-gray-800 transition w-full mt-4">娣囨繂鐡ㄩ柊宥囩枂</button></div></div></div><div v-if="sa" class="modal-overlay"><div class="bg-white p-8 rounded-2xl w-[420px] shadow-2xl border border-gray-100" style="width:420px"><h3 class="text-2xl font-bold mb-6 tracking-tight">{{isEdit?'缂傛牞绶€广垺鍩?:'濞ｈ濮炵€广垺鍩?}}</h3><div class="space-y-4"><div><label class="block text-sm font-bold text-gray-500 mb-1">閻劍鍩涢崥?/label><input v-model="nu.username" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div><label class="block text-sm font-bold text-gray-500 mb-1">鐎靛棛鐖?/label><div class="flex gap-2"><input v-model="nu.password" class="flex-1 p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black font-mono"><button @click="nu.password=gp()" class="px-3 bg-gray-200 rounded hover:bg-gray-300 text-sm font-bold">闂呭繑婧€</button></div></div><div><label class="block text-sm font-bold text-gray-500 mb-1">濞翠線鍣洪柊宥夘杺 (GB閿?=閺冪娀妾?</label><input v-model.number="nu.LimitGb" type="number" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div><label class="block text-sm font-bold text-gray-500 mb-1">鏉╁洦婀￠弮鍫曟？ (缁?濮橀晲绠?</label><input v-model="nu.ExpStr" type="date" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black text-gray-600"></div><div class="flex items-center justify-between mt-4"><span class="font-bold text-gray-700">閸氼垳鏁?/span><button @click="nu.enable=!nu.enable" :class="nu.enable?'bg-green-500':'bg-gray-300'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" style="width:44px;height:24px;border-radius:12px;border:none;cursor:pointer"><span :class="nu.enable?'translate-x-6':'translate-x-1'" class="inline-block h-4 w-4 transform rounded-full bg-white transition" style="display:inline-block;width:16px;height:16px;background:white;border-radius:50%;transition:transform 0.2s;transform:translateX(nu.enable?24px:4px)"></span></button></div><div class="flex gap-4 mt-8" style="display:flex;gap:1rem"><button @click="au" class="flex-1 bg-black text-white p-3 rounded-lg font-bold shadow hover:bg-gray-800 transition" style="flex:1">绾喛顓绘穱婵嗙摠</button><button @click="sa=false" class="flex-1 bg-gray-100 text-gray-600 p-3 rounded-lg font-bold hover:bg-gray-200 transition" style="flex:1;background:#f3f4f6">閸欐牗绉?/button></div></div></div></div><div v-if="scv" class="modal-overlay" @click.self="scv=false"><div class="bg-white p-8 rounded-2xl shadow-2xl border border-gray-100" style="width:520px;max-width:90vw"><h3 class="text-xl font-bold mb-4">棣冩惖 鐎广垺鍩涚粩顖炲帳缂?閳?{{scu.username}}</h3><div class="space-y-4"><div><div class="flex justify-between items-center mb-1"><span class="text-sm font-bold text-gray-500">VLESS 闁剧偓甯?(App 鐎电厧鍙?</span><button @click="doCopy(scvless)" class="copy-btn bg-blue-500 text-white hover:bg-blue-600">婢跺秴鍩?/button></div><div class="config-box">{{scvless}}</div></div><div><div class="flex justify-between items-center mb-1"><span class="text-sm font-bold text-gray-500">JSON 闁板秶鐤?(鏉烆垵鐭鹃悽鍗烆嚤閸?</span><button @click="doCopy(scjson)" class="copy-btn bg-purple-500 text-white hover:bg-purple-600">婢跺秴鍩?/button></div><div class="config-box">{{scjson}}</div></div><button @click="scv=false" class="w-full mt-4 bg-gray-100 text-gray-600 p-3 rounded-lg font-bold hover:bg-gray-200 transition">閸忔娊妫?/button></div></div></div><div v-if="toast" class="toast">{{toast}}</div></template></div><script>Vue.createApp({data(){return{log:false,t:'s',sa:false,isEdit:false,u:'',p:'',st:{cpu:'-',mem:'-',go:'-',ut:'-',tx:'-',rx:'-'},us:[],cf:{},nu:{enable:true,LimitGb:0,ExpStr:''},scv:false,scu:{},scvless:'',scjson:'',toast:''}},methods:{uuidFor(s){let h=[2166136261,2166136261^0x9e3779b9,2166136261^0x85ebca6b,2166136261^0xc2b2ae35];for(let j=0;j<h.length;j++){for(let i=0;i<s.length;i++){h[j]^=s.charCodeAt(i)+j*31;h[j]=Math.imul(h[j],16777619)}}let hex='';for(let k=0;k<h.length;k++){hex+=(h[k]>>>0).toString(16).padStart(8,'0')}hex=hex.slice(0,12)+'4'+hex.slice(13);hex=hex.slice(0,16)+'8'+hex.slice(17);return hex.slice(0,8)+'-'+hex.slice(8,12)+'-'+hex.slice(12,16)+'-'+hex.slice(16,20)+'-'+hex.slice(20,32)},async req(p,m='GET',b){let o={method:m};if(b){o.body=JSON.stringify(b);o.headers={'Content-Type':'application/json'}};try{let r=await fetch('/api/'+p,o);if(r.status===401){this.log=false;throw new Error("401")}return await r.json()}catch(e){throw e}},async li(){try{let r=await this.req('login','POST',{User:this.u,Pass:this.p});if(r&&r.ok){this.log=true;this.fs()}}catch(e){alert('閻ц缍嶆径杈Е')}},async fs(){try{let r=await this.req('status');if(r)this.st=r}catch(e){}},async fu(){try{let r=await this.req('users');if(r)this.us=r}catch(e){}},async fc(){try{let r=await this.req('settings');if(r)this.cf=r}catch(e){}},gp(){const c='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789';let r='';for(let i=0;i<16;i++)r+=c[Math.floor(Math.random()*c.length)];return r},async rg(){let nu={username:'user_'+Math.random().toString(36).slice(2,8),password:this.gp(),enable:true,LimitGb:0,expire_at:0};await this.req('users','POST',nu);this.fu();this.showToast('瀹歌尙鏁撻幋? '+nu.username)},eu(u){this.nu=JSON.parse(JSON.stringify(u));if(u.expire_at>0){this.nu.ExpStr=new Date(u.expire_at*1000).toISOString().split('T')[0]}else{this.nu.ExpStr=''}this.nu.LimitGb=u.limit/1073741824;this.isEdit=true;this.sa=true},async au(){if(!this.nu.username||!this.nu.password){alert('鐠愶箑褰跨€靛棛鐖滆箛鍛綖');return}this.nu.limit=this.nu.LimitGb*1073741824;if(this.nu.ExpStr){this.nu.expire_at=new Date(this.nu.ExpStr).getTime()/1000}else{this.nu.expire_at=0}if(this.isEdit){await this.req('users/edit','POST',this.nu)}else{await this.req('users','POST',this.nu)}this.sa=false;this.fu()},async du(id){if(confirm('绾喖鐣鹃崚鐘绘珟?')){await this.req('users/del','POST',{ID:id});this.fu()}},cp(u,type){this.scu=u;let srv=this.cf.domain||location.hostname;let port=this.cf.listen_ports||'8443';let sni=this.cf.domain||'';if(type==='vless'){this.scvless='vless://'+this.uuidFor(u.username+':'+u.password)+'@'+srv+':'+port+'?encryption=none&security='+(sni?'tls':'none')+(sni?'&sni='+sni:'')+'&type=tcp&headerType=none#'+u.username}else{this.scjson=JSON.stringify({enable:true,active_node_id:'n_main',nodes:[{id:'n_main',name:u.username,server:srv+':'+port,username:u.username,password:u.password,sni:sni}]},null,2)}this.scv=true},doCopy(text){navigator.clipboard.writeText(text).then(()=>{this.showToast('瀹告彃顦查崚璺哄煂閸擃亣鍒涢弶?)}).catch(()=>{let ta=document.createElement('textarea');ta.value=text;document.body.appendChild(ta);ta.select();document.execCommand('copy');document.body.removeChild(ta);this.showToast('瀹告彃顦查崚?)})},showToast(msg){this.toast=msg;setTimeout(()=>{this.toast=''},2000)},async sc(){await this.req('settings','POST',this.cf);this.showToast('闁板秶鐤嗗韫箽鐎?)}},mounted(){this.req('status').then(r=>{if(r){this.log=true;this.fs();this.fc();setInterval(()=>{if(this.t=='s')this.fs();if(this.t=='u')this.fu()},3000)}}).catch(e=>{})}}).mount('#app')</script></body></html>
`
